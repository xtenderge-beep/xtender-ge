...
Действующая редакция обоих документов — **`v1.2-2026-09-03`**. Оферта покрывает: статус
сервиса как информпосредника (не работодатель/агент/налоговый агент), налоги на
исполнителе, AS IS + отказ от ответственности, **раздел 1.3** — внутренний баланс/аванс,
мин. 5 ₾, автосписание за факт отправки лида, **отсутствие функции «Пауза»**, No-Refund,
**ценз 18+**, персональные данные, право Грузии, приоритет грузинской версии. Политика
покрывает: какие данные собираются (вкл. cookie), цели, публичность номера в каталоге,
сроки хранения, право на забвение (**обработка ≤ 10 рабочих дней**), передача третьим
лицам, Data Controller.
...
| Подзаголовок + инфо-блок про оплату на `/join` | `src/config/i18n.js` | `join_subtitle`, `join_payment_note` (блок над чекбоксами в `src/views/join.ejs`) |
...
списывает 50 тетри за каждое отправленное ему уведомление о заявке. Кончился баланс —
заявки просто перестают приходить (без минуса, без долга). SMS отправляет xtender, за
доставку оператору платит xtender.

Оферта п. 1.3 описывает это как «внутренний лицевой счёт (аванс)». Формулировки на `/join`
(`join_subtitle`, `join_terms_label`, `join_payment_note`) явно говорят «не с телефонного
счёта» — правились 2026-09-03, чтобы не читалось как премиум-SMS.
...
- **Тариф лида.** Текст оферты цену не называет («согласно тарифу»). Код списывает
  **50 тетри** (0.50 GEL) — `COST_PER_NOTIFICATION_TETRI` в `order.service.js`,
  ...
  строки i18n `join_subtitle` / `join_payment_note` / `join_payment_desc` /
  `meta_description_join` ×3 языка. Изменён с 30 → 50 тетри 2026-09-03. Все эти места
  держать в синхроне.
