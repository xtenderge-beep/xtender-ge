...
**поднимать при любом изменении** соответствующего текста в `legal-content.js` или
текста чекбокса в `i18n.js`. По этой строке в журнале согласий видно, какую именно
редакцию принял конкретный человек. Понизить/переиспользовать версию нельзя.
...
- **Тариф лида.** Текст оферты цену не называет («согласно тарифу»). Код списывает
  **30 тетри** (`COST_PER_NOTIFICATION_TETRI` в `order.service.js`, `LEAD_PRICE_TETRI`
  ×2 в контроллерах, строки i18n «30 тетри»). Если меняется на 50 — отдельная правка.
