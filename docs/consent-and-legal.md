...
Чекбокс Оферты на `/join` нельзя поставить напрямую кликом — клик открывает модалку
с полным текстом документа (`partials/legal-blocks.ejs`, тот же рендер, что на `/terms`).
Кнопка «Прочитал(а) и принимаю» внутри неё задизейблена, пока `termsModalBody` не
проскроллен до конца (`join.ejs`, `checkTermsScrollEnd()`). Только после этого чекбокс
ставится программно.
