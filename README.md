...
Модель оплаты — за лид: каждое SMS-уведомление о заявке списывается с баланса мастера (`masters.balance_tetri`, в тетри — 1 GEL = 100 тетри), сейчас **50 тетри** (0.50 GEL) — **цена настраивается в `/admin/settings`** (`app_settings.lead_price_tetri`, читается `settingsService.getLeadPriceTetri()`; дефолт 50 в коде — только фолбэк, если строки в БД нет). Списание — за сам факт отправки уведомления, не за исход (звонок/ответ клиенту не проверяется и не требуется). Если баланса не хватает — мастера просто пропускают при рассылке (без ухода в минус). Пополнение баланса — вручную: исполнитель переводит деньги (минимум 5 GEL) по договорённости, модератор подтверждает командой прямо в Telegram-боте:
...
- **app_settings** — key/value настройки приложения, правятся в `/admin/settings` (сейчас один ключ — `lead_price_tetri`, цена лида)
