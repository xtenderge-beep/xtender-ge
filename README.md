...
Модель оплаты — за лид: каждое SMS-уведомление о заявке стоит **50 тетри** (0.50 GEL), списывается с баланса мастера (`masters.balance_tetri`, в тетри — 1 GEL = 100 тетри). Если баланса не хватает — мастера просто пропускают при рассылке (без ухода в минус). Пополнение баланса — вручную: исполнитель переводит деньги (минимум 5 GEL) по договорённости, модератор подтверждает командой прямо в Telegram-боте:
