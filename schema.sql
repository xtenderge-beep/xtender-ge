...
ALTER TABLE masters ADD CONSTRAINT masters_vehicle_size_check
    CHECK (vehicle_size IS NULL OR vehicle_size IN ('S', 'M', 'L', 'XL', 'XXL'));
...
CREATE TABLE IF NOT EXISTS sms_consent_logs (
    ...
    -- terms_version: действующая версия оферты (src/config/legal.js) на момент события —
...
-- Shared service catalog. Stable keys preserve profile and dispatch history.
CREATE TABLE IF NOT EXISTS service_categories (
 slug VARCHAR(32) PRIMARY KEY,
 name_ka VARCHAR(80) NOT NULL, name_ru VARCHAR(80) NOT NULL, name_en VARCHAR(80) NOT NULL,
 icon VARCHAR(16) NOT NULL DEFAULT '', fields JSONB NOT NULL DEFAULT '[]'::jsonb,
 is_active BOOLEAN NOT NULL DEFAULT true, is_builtin BOOLEAN NOT NULL DEFAULT false,
 sort_order INTEGER NOT NULL DEFAULT 100, version INTEGER NOT NULL DEFAULT 1
);
INSERT INTO service_categories(slug,name_ka,name_ru,name_en,icon,fields,is_builtin,sort_order) VALUES('van','გადაზიდვა','Перевозки','Transport','🚚','[{"key":"size","input":"size","options":["S","M","L","XL","XXL"],"match":"gte","filter":true,"labels":{"ka":"ძარის ზომა","ru":"Размер кузова","en":"Cargo size"},"optionLabels":{"S":{"ka":"S","ru":"S","en":"S"},"M":{"ka":"M","ru":"M","en":"M"},"L":{"ka":"L","ru":"L","en":"L"},"XL":{"ka":"XL","ru":"XL","en":"XL"},"XXL":{"ka":"XXL","ru":"XXL","en":"XXL"}}},{"key":"body","input":"enum","options":["closed","flatbed"],"match":"exact","filter":true,"required":true,"optionIcons":{"closed":"fa-truck","flatbed":"fa-truck-pickup"},"labels":{"ka":"ძარის ტიპი","ru":"Тип кузова","en":"Body type"},"optionLabels":{"closed":{"ka":"დახურული","ru":"Закрытый","en":"Enclosed"},"flatbed":{"ka":"ბორტიანი (ღია)","ru":"Борт (открытый)","en":"Flatbed (open)"}}},{"key":"tail_lift","input":"bool","match":"flag","filter":true,"labels":{"ka":"ჰიდრობორტი (ამწე)","ru":"Гидроборт (подъёмник)","en":"Tail lift"},"optionLabels":{}},{"key":"with_helpers","input":"bool","match":"flag","filter":true,"labels":{"ka":"ჩამოვდივარ დამტვირთავებთან ერთად","ru":"Приезжаю с грузчиками","en":"I bring loaders"},"optionLabels":{}}]',true,1) ON CONFLICT(slug) DO NOTHING;
-- 'van' — единственная категория, чьи характеристики закрыты для редактирования в
-- /admin/categories (см. category.service.hasLockedField: у неё есть поле 'size',
-- которое ставится не общим движком, а legacy-колонкой vehicle_size). Раз админ не
-- может случайно её испортить через форму, безопасно держать fields в синхроне с
-- кодом на каждом старте — иначе добавленный здесь тир M не попал бы на уже
-- существующие базы (INSERT ... ON CONFLICT DO NOTHING их не трогает).
UPDATE service_categories SET fields='[{"key":"size","input":"size","options":["S","M","L","XL","XXL"],"match":"gte","filter":true,"labels":{"ka":"ძარის ზომა","ru":"Размер кузова","en":"Cargo size"},"optionLabels":{"S":{"ka":"S","ru":"S","en":"S"},"M":{"ka":"M","ru":"M","en":"M"},"L":{"ka":"L","ru":"L","en":"L"},"XL":{"ka":"XL","ru":"XL","en":"XL"},"XXL":{"ka":"XXL","ru":"XXL","en":"XXL"}}},{"key":"body","input":"enum","options":["closed","flatbed"],"match":"exact","filter":true,"required":true,"optionIcons":{"closed":"fa-truck","flatbed":"fa-truck-pickup"},"labels":{"ka":"ძარის ტიპი","ru":"Тип кузова","en":"Body type"},"optionLabels":{"closed":{"ka":"დახურული","ru":"Закрытый","en":"Enclosed"},"flatbed":{"ka":"ბორტიანი (ღია)","ru":"Борт (открытый)","en":"Flatbed (open)"}}},{"key":"tail_lift","input":"bool","match":"flag","filter":true,"labels":{"ka":"ჰიდრობორტი (ამწე)","ru":"Гидроборт (подъёмник)","en":"Tail lift"},"optionLabels":{}},{"key":"with_helpers","input":"bool","match":"flag","filter":true,"labels":{"ka":"ჩამოვდივარ დამტვირთავებთან ერთად","ru":"Приезжаю с грузчиками","en":"I bring loaders"},"optionLabels":{}}]'::jsonb WHERE slug='van';
INSERT INTO service_categories(slug,name_ka,name_ru,name_en,icon,fields,is_builtin,sort_order) VALUES('movers','მუშები','Грузчики','Movers','💪','[{"key":"crew_size","input":"number","unit":"чел.","min":1,"max":20,"match":"gte","required":true,"labels":{"ka":"ბრიგადა","ru":"Бригада","en":"Crew size"},"optionLabels":{}}]',true,2) ON CONFLICT(slug) DO NOTHING;
...
INSERT INTO service_categories(slug,name_ka,name_ru,name_en,icon,fields,is_builtin,sort_order) VALUES('junk','ნარჩენების გატანა (თვითმცლელით)','Вывоз мусора (самосвал)','Waste removal (dump truck)','🧹','[{"key":"volume_m3","input":"enum","options":["2","4","8","15"],"unit":"м³","match":"gte","filter":true,"required":true,"labels":{"ka":"ძარის მოცულობა","ru":"Объём кузова","en":"Cargo volume"},"optionLabels":{"2":{"ka":"2 მ³-მდე","ru":"до 2 м³","en":"up to 2 m³"},"4":{"ka":"4 მ³-მდე","ru":"до 4 м³","en":"up to 4 m³"},"8":{"ka":"8 მ³-მდე","ru":"до 8 м³","en":"up to 8 m³"},"15":{"ka":"15+ მ³","ru":"15+ м³","en":"15+ m³"}}}]',true,5) ON CONFLICT(slug) DO NOTHING;
-- Раньше 'junk' покрывал и самосвалы, и вывоз вручную мешками — у них разная
-- вместимость (у второго её просто нет), поэтому завели объём кузова именно тут и
-- завели отдельную категорию ниже для ручного вывоза, а не смешивали оба варианта в
-- одних характеристиках. Гвардия по fields='[]' — чтобы не затереть, если админ уже
-- сам отредактировал эту (не залоченную, в отличие от 'van') категорию через форму.
